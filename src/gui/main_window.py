...
class MainWindow(ctk.CTk):
    ...
    def create_sidebar(self):
        ...
        self.char_name_entry = ctk.CTkEntry(
            self.sidebar_frame, placeholder_text="Name / Alias"
        )
        ...
        self.gallery_frame = ctk.CTkScrollableFrame(
            self.sidebar_frame, height=120, fg_color="#222"
        )
        ...
        ctk.CTkLabel(
            self.sidebar_frame,
            text="Google Gemini API Key (Vision)",
        ...
        ctk.CTkLabel(
            self.sidebar_frame,
            text="Google Gemini API Key (Writer)",
            font=ctk.CTkFont(size=10),
        ).pack(anchor="w", padx=10)
        ...
        ctk.CTkLabel(
            self.sidebar_frame,
            text="Google Imagen API Key (Images)",
            font=ctk.CTkFont(size=10),
        ).pack(anchor="w", padx=10)
        ...
        # Reset Button
        ctk.CTkFrame(
            self.sidebar_frame, height=20, fg_color="transparent"
        ).pack()  # Spacer
    ...
    def create_split_tab(self, tab_name, header_text, section_key):
        ...
        for category, items in section_data.items():
            # Card Frame
            card = ctk.CTkFrame(
                options_frame, fg_color=THEME["card_bg"], corner_radius=10
            )
    ...
    def start_simulation(self):
        ...
        if not writer_key:
            messagebox.showwarning("Chat", "Bitte Writer API Key konfigurieren.")
            return

        if not self.ai_client.validate_api_key(writer_key):
            messagebox.showerror("Chat", "Writer API Key ist ungültig.")
            return

        configure_result = self.ai_client.configure(writer_key)
        if not configure_result.success:
            messagebox.showerror("Verbindung", configure_result.error)
            return
        ...
        chat_result = self.ai_client.start_chat(system_prompt)
        if not chat_result.success:
            messagebox.showerror("Chat", chat_result.error)
            return

        self.chat_session = chat_result.data
    ...
    def send_chat_message(self, event=None):
        if not self.chat_session:
            messagebox.showwarning("Chat", "Bitte zuerst eine Simulation starten.")
            return
        ...
        try:
            response = self.chat_session.send_message(
                msg,
                request_options={"timeout": self.ai_client.request_timeout},
            )
            ...
            self.chat_history._textbox.insert("end", f"Fehler: {e}\n", "system")
    ...
    def generate_visual(self):
        ...
        if not self.ai_client.validate_api_key(image_key):
            messagebox.showerror("Fehler", "Imagen API Key scheint ungültig.")
            return
        ...
        configure_result = self.ai_client.configure(
            writer_key
        )  # Ensure Gemini is ready for translation
        if not configure_result.success:
            messagebox.showerror("Verbindung", configure_result.error)
            return
        ...
        try:
            text_result = self.ai_client.generate_text(translation_prompt)
            if not text_result.success:
                messagebox.showerror("Fehler", text_result.error)
                return
            image_prompt = text_result.data

            # Generate with Imagen 4.0
            image_result = self.ai_client.generate_image(image_prompt, image_key)

            if image_result.success:
                # result_image is the generated image object from google-genai
                try:
                    result_image = image_result.data
                    ...
                    self.image_container.configure(
                        image=self.generated_image_ctk, text=""
                    )
                ...
                messagebox.showerror(
                    "Fehler", f"Bildgenerierung fehlgeschlagen: {image_result.error}"
                )
    ...
    def save_and_connect(self):
        ...
        invalid_keys = [
            key
            for key in [vision_key, writer_key, image_key]
            if not self.ai_client.validate_api_key(key)
        ]
        if invalid_keys:
            messagebox.showerror(
                "Konfiguration",
                "Mindestens einer der API-Schlüssel ist ungültig (zu kurz oder leer).",
            )
            return
        ...
        try:
            with open(self.config_file, "w") as f:
                json.dump(config, f)
            connection_result = self.ai_client.configure(writer_key)
            if connection_result.success:
                messagebox.showinfo(
                    "Erfolg", "Keys gespeichert und Writer-Modell verbunden!"
                )
            else:
                messagebox.showerror("Verbindung", connection_result.error)
        except Exception as e:
    ...
    def _prefill_keys_from_env(self):
        env_keys = {
            "vision_key": os.getenv("GEMINI_VISION_KEY") or os.getenv("GEMINI_API_KEY"),
            "writer_key": os.getenv("GEMINI_WRITER_KEY") or os.getenv("GEMINI_API_KEY"),
            "image_key": os.getenv("IMAGEN_API_KEY")
            or os.getenv("GOOGLE_IMAGE_API_KEY"),
        }

        if not self.vision_api_entry.get() and env_keys["vision_key"]:
            self.vision_api_entry.insert(0, env_keys["vision_key"])
        if not self.writer_api_entry.get() and env_keys["writer_key"]:
            self.writer_api_entry.insert(0, env_keys["writer_key"])
        if not self.image_api_entry.get() and env_keys["image_key"]:
            self.image_api_entry.insert(0, env_keys["image_key"])
        ...
        if os.path.exists(filepath):
            try:
                ...
                if hasattr(self, "physiology_input"):
                    ...
                    self.physiology_input.insert(
                        "1.0", self.character.physiology.content
                    )
                if hasattr(self, "psychology_input"):
                    self.psychology_input.delete("1.0", "end")
                    self.psychology_input.insert(
                        "1.0", self.character.psychology.content
                    )
                ...
                if hasattr(self, "gem_matrix_input"):
                    ...
                    self.gem_matrix_input.insert(
                        "1.0", self.character.gem_matrix.content
                    )
                all_options = {
    ...
    def sync_ui_to_model(self):
        if hasattr(self, "physiology_input"):
            self.character.physiology.content = self.physiology_input.get(
                "1.0", "end-1c"
            )
        if hasattr(self, "psychology_input"):
            self.character.psychology.content = self.psychology_input.get(
                "1.0", "end-1c"
            )
        if hasattr(self, "sensory_input"):
        ...
        if hasattr(self, "gem_matrix_input"):
            self.character.gem_matrix.content = self.gem_matrix_input.get(
                "1.0", "end-1c"
            )
    ...
    def generate_description(self, section_key):
        ...
        if not self.ai_client.validate_api_key(writer_key):
            messagebox.showerror("KI Warnung", "Writer API Key ist ungültig.")
            return

        # Vision Phase
        vision_summary = "Keine Bilder vorhanden."
        if self.uploaded_images and vision_key:
            vision_config = self.ai_client.configure(vision_key)
            if not vision_config.success:
                messagebox.showerror("Verbindung", vision_config.error)
                return
            vision_prompt = (
                "Du bist ein forensischer Analyst. Beschreibe NUR das Physische..."
            )
            try:
                vision_result = self.ai_client.analyze_images(
                    self.uploaded_images, vision_prompt
                )
                if vision_result.success:
                    vision_summary = vision_result.data
                else:
                    messagebox.showwarning("Vision", vision_result.error)
            except:
                pass

        # Writer Phase
        writer_config = self.ai_client.configure(writer_key)
        if not writer_config.success:
            messagebox.showerror("Verbindung", writer_config.error)
            return
        selections = {}
        ...
        """
        try:
            generated_result = self.ai_client.generate_text(
                writer_prompt, temperature=0.7
            )
            if not generated_result.success:
                messagebox.showerror("Generierungsfehler", generated_result.error)
                return

            generated_text = generated_result.data
