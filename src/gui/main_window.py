...
class Tooltip:
    ...
    def show_tooltip(self, event=None):
        ...
        label = ctk.CTkLabel(self.tooltip_window, text=self.text, fg_color="#333", corner_radius=5, padx=10, pady=5)
        label.pack()
...
class MainWindow(ctk.CTk):
    def __init__(self):
        ...
        self.tabview = ctk.CTkTabview(self, fg_color=THEME["bg_dark"], segmented_button_fg_color=THEME["card_bg"], segmented_button_selected_color=THEME["accent"])
        self.tabview.grid(row=0, column=1, padx=(20, 20), pady=(20, 20), sticky="nsew")
        
        self.tabs = ["Physiologie", "Psychologie", "Sensorik", "Historie", "GEM-Matrix", "Dossier", "Chat-Labor"]
        ...
        # Create Tabs
        self.create_split_tab("Physiologie", "I. Physiologische Mikroanalyse", "physiology")
        self.create_split_tab("Psychologie", "II. Psycho-Neurologische Tiefenanalyse", "psychology")
        self.create_split_tab("Sensorik", "III. Sensorische Mikroprofile", "sensory")
        self.create_split_tab("Historie", "IV. Entwicklungsgeschichte", "history")
        self.create_split_tab("GEM-Matrix", "XI. GEM V1.3 STEUERUNGS-MATRIX", "gem_matrix")
    ...
    def create_sidebar(self):
        self.sidebar_frame = ctk.CTkScrollableFrame(self, width=300, corner_radius=0, fg_color=THEME["card_bg"], label_text="KONTROLLZENTRUM")
        ...
        # Age Slider
        ctk.CTkLabel(self.sidebar_frame, text="Alter:", font=ctk.CTkFont(size=12)).pack(pady=(5, 0), anchor="w", padx=10)
        self.age_slider = ctk.CTkSlider(self.sidebar_frame, from_=18, to=70, number_of_steps=52, button_color=THEME["accent"], progress_color=THEME["accent"])
        self.age_slider.set(25)
        self.age_slider.pack(pady=5, padx=10, fill="x")
        self.age_label = ctk.CTkLabel(self.sidebar_frame, text="25 Jahre", text_color=THEME["neon_pink"])
        self.age_label.pack(pady=0)
        self.age_slider.configure(command=lambda v: self.age_label.configure(text=f"{int(v)} Jahre"))
        ...
        self.upload_btn = ctk.CTkButton(self.sidebar_frame, text="Bilder Upload (+)", command=self.upload_image, fg_color=THEME["accent"], hover_color=THEME["accent_hover"])
        self.upload_btn.pack(pady=5, padx=10, fill="x")
        
        # API Config
        self.create_sidebar_section("SYSTEM LINK")
        
        ctk.CTkLabel(self.sidebar_frame, text="Google Gemini API Key (Vision)", font=ctk.CTkFont(size=10)).pack(anchor="w", padx=10)
        self.vision_api_entry = ctk.CTkEntry(self.sidebar_frame, placeholder_text="Vision Key", show="*")
        self.vision_api_entry.pack(pady=(0, 5), padx=10, fill="x")

        ctk.CTkLabel(self.sidebar_frame, text="Google Gemini API Key (Writer)", font=ctk.CTkFont(size=10)).pack(anchor="w", padx=10)
        self.writer_api_entry = ctk.CTkEntry(self.sidebar_frame, placeholder_text="Writer Key", show="*")
        self.writer_api_entry.pack(pady=(0, 5), padx=10, fill="x")
        
        ctk.CTkLabel(self.sidebar_frame, text="Google Imagen API Key (Images)", font=ctk.CTkFont(size=10)).pack(anchor="w", padx=10)
        self.image_api_entry = ctk.CTkEntry(self.sidebar_frame, placeholder_text="Imagen Key", show="*")
        self.image_api_entry.pack(pady=(0, 5), padx=10, fill="x")
        
        self.save_config_btn = ctk.CTkButton(self.sidebar_frame, text="System Verbinden", command=self.save_and_connect, fg_color="#10B981")
        self.save_config_btn.pack(pady=5, padx=10, fill="x")

        # Dossier Management
        self.create_sidebar_section("SPEICHER")
        self.dossier_list = ctk.CTkComboBox(self.sidebar_frame, values=["Lade..."], command=self.load_selected_dossier)
        self.dossier_list.pack(pady=5, padx=10, fill="x")
        self.refresh_dossiers()
        
        self.save_dossier_btn = ctk.CTkButton(self.sidebar_frame, text="Dossier Speichern", command=self.save_dossier, fg_color="#F59E0B")
        ...
        self.reset_btn = ctk.CTkButton(self.sidebar_frame, text="♻ NEUER CHARAKTER", command=self.reset_character, fg_color=THEME["danger"], hover_color="#b91c1c")
        self.reset_btn.pack(pady=20, padx=10, fill="x")

    def create_sidebar_section(self, title):
        ctk.CTkLabel(self.sidebar_frame, text=title, font=ctk.CTkFont(size=11, weight="bold"), text_color=THEME["text_sub"]).pack(pady=(15, 5), anchor="w", padx=10)
    ...
    def create_split_tab(self, tab_name, header_text, section_key):
        ...
        # Header
        lbl = ctk.CTkLabel(tab, text=header_text, font=ctk.CTkFont(size=20, weight="bold"), text_color=THEME["neon_blue"])
        ...
        if not hasattr(self, 'option_widgets'): self.option_widgets = {}
        ...
        for category, items in section_data.items():
            ...
            # Card Header
            ctk.CTkLabel(card, text=category, font=ctk.CTkFont(size=14, weight="bold"), text_color=THEME["accent"]).pack(pady=10, padx=10, anchor="w")
            ...
            for option_name, values in items.items():
                ...
                ctk.CTkLabel(label_frame, text=option_name, font=ctk.CTkFont(size=12)).pack(side="left")
                info_lbl = ctk.CTkLabel(label_frame, text="ⓘ", font=ctk.CTkFont(size=12), text_color=THEME["text_sub"], cursor="hand2")
                info_lbl.pack(side="left", padx=5)
                Tooltip(info_lbl, f"Bestimmt {option_name} des Charakters.") # Simple dynamic tooltip
                
                # ComboBox
                combo = ctk.CTkComboBox(row, values=values, width=200, fg_color="#222", border_color="#444", button_color=THEME["accent"])
        ...
        textbox = ctk.CTkTextbox(text_frame, font=ctk.CTkFont(size=13), fg_color="#111", text_color="#eee")
        textbox.grid(row=0, column=0, sticky="nsew", padx=10, pady=10)
        setattr(self, f"{section_key}_input", textbox)
        
        # Generate Button
        gen_btn = ctk.CTkButton(text_frame, text="✨ GENERIEREN", command=lambda s=section_key: self.generate_description(s), fg_color=THEME["accent"], height=40, font=ctk.CTkFont(weight="bold"))
    ...
    def create_dossier_tab(self):
        ...
        # Image Container
        self.image_container = ctk.CTkLabel(container, text="[Kein Bild generiert]", height=300, fg_color="#111", corner_radius=10)
        self.image_container.pack(fill="x", pady=(0, 10))
        
        self.dossier_preview = ctk.CTkTextbox(container, font=ctk.CTkFont(size=13), fg_color="#111", height=500)
        self.dossier_preview.pack(fill="both", expand=True)
        
        btn_frame = ctk.CTkFrame(tab, fg_color="transparent")
        btn_frame.grid(row=1, column=0, sticky="ew", padx=10, pady=10)
        
        ctk.CTkButton(btn_frame, text="🔄 Dossier Aktualisieren", command=self.update_dossier_preview, fg_color=THEME["accent"]).pack(side="left", padx=5)
        ctk.CTkButton(btn_frame, text="🖼 BILD GENERIEREN", command=self.generate_visual, fg_color=THEME["neon_pink"]).pack(side="left", padx=5)
        ctk.CTkButton(btn_frame, text="JSON Export", command=self.save_dossier, fg_color="#F59E0B").pack(side="right", padx=5)
    ...
    def create_chat_tab(self):
        ...
        self.chat_history = ctk.CTkTextbox(tab, font=ctk.CTkFont(size=13), fg_color="#111", state="disabled")
        ...
        ctk.CTkButton(input_frame, text="Senden", command=self.send_chat_message, fg_color=THEME["accent"], width=100).pack(side="right")
        
        ctk.CTkButton(tab, text="▶ SIMULATION STARTEN", command=self.start_simulation, fg_color=THEME["neon_blue"]).grid(row=2, column=0, pady=10)
    ...
    def send_chat_message(self, event=None):
        ...
        try:
            ...
            self.chat_history._textbox.insert("end", f"{self.char_name_entry.get()}: {response.text}\n\n", "model")
        ...
        try:
            ...
            if success:
                ...
                try:
                    ...
                    self.generated_image_ctk = ctk.CTkImage(light_image=img, dark_image=img, size=img.size)
    ...
    def upload_image(self):
        if len(self.uploaded_images) >= 10:
            ...
            return
        filepaths = filedialog.askopenfilenames(title="Charakterbilder auswählen", filetypes=[("Image files", "*.jpg *.jpeg *.png *.webp")])
        for filepath in filepaths:
            if len(self.uploaded_images) >= 10: break
    ...
    def add_thumbnail(self, filepath):
        try:
            ...
            ctk.CTkButton(frame, text="X", width=20, fg_color=THEME["danger"], command=lambda p=filepath, f=frame: self.remove_image(p, f)).pack(side="right", padx=5)
    ...
    def save_and_connect(self):
        ...
        try:
            ...
            self.ai_client.configure(writer_key) # Configure main client immediately with writer key
        except Exception as e: messagebox.showerror("Fehler", f"Konnte Config nicht speichern: {e}")
        ...
        if os.path.exists(self.config_file):
            try:
                with open(self.config_file, "r") as f:
                    config = json.load(f)
                    self.vision_api_entry.insert(0, config.get("vision_key", "") or config.get("gemini_key", ""))
                    self.writer_api_entry.insert(0, config.get("writer_key", "") or config.get("gemini_key", ""))
                    self.image_api_entry.insert(0, config.get("image_key", ""))
            except Exception: pass

    def refresh_dossiers(self):
        files = [f.replace(".json", "") for f in os.listdir(self.saved_dossiers_dir) if f.endswith(".json")]
    ...
    def load_selected_dossier(self, name):
        ...
        if os.path.exists(filepath):
            try:
                ...
                if hasattr(self, 'physiology_input'): self.physiology_input.delete("1.0", "end"); self.physiology_input.insert("1.0", self.character.physiology.content)
                if hasattr(self, 'psychology_input'): self.psychology_input.delete("1.0", "end"); self.psychology_input.insert("1.0", self.character.psychology.content)
                if hasattr(self, 'sensory_input'): self.sensory_input.delete("1.0", "end"); self.sensory_input.insert("1.0", self.character.sensory.content)
                if hasattr(self, 'history_input'): self.history_input.delete("1.0", "end"); self.history_input.insert("1.0", self.character.history.content)
                if hasattr(self, 'gem_matrix_input'): self.gem_matrix_input.delete("1.0", "end"); self.gem_matrix_input.insert("1.0", self.character.gem_matrix.content)
                all_options = {**self.character.physiology_options, **self.character.psychology_options, **self.character.sensory_options, **self.character.history_options, **self.character.gem_options}
                for section_widgets in self.option_widgets.values():
                    for key, widget in section_widgets.items():
                        if key in all_options: widget.set(all_options[key])
                messagebox.showinfo("Erfolg", f"Dossier '{name}' geladen.")
            except Exception as e: messagebox.showerror("Fehler", f"Laden fehlgeschlagen: {e}")

    def sync_ui_to_model(self):
        if hasattr(self, 'physiology_input'): self.character.physiology.content = self.physiology_input.get("1.0", "end-1c")
        if hasattr(self, 'psychology_input'): self.character.psychology.content = self.psychology_input.get("1.0", "end-1c")
        if hasattr(self, 'sensory_input'): self.character.sensory.content = self.sensory_input.get("1.0", "end-1c")
        if hasattr(self, 'history_input'): self.character.history.content = self.history_input.get("1.0", "end-1c")
        if hasattr(self, 'gem_matrix_input'): self.character.gem_matrix.content = self.gem_matrix_input.get("1.0", "end-1c")
    ...
    def generate_description(self, section_key):
        ...
        for key, widget in widgets.items():
            val = widget.get()
            if val not in ["Keine Auswahl", "None"]: selections[key] = val
        ...
        except Exception as e: messagebox.showerror("Generierungsfehler", str(e))
